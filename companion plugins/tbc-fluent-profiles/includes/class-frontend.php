...
class Frontend {
    ...
    public function inject_css() {
        ?>
        <link rel="stylesheet"
              href="<?php echo esc_url(TBC_FP_URL . 'assets/css/profile-fields.css'); ?>?v=<?php echo esc_attr(TBC_FP_VERSION); ?>"
              media="screen"/>
        <?php
    }
    ...
    public function inject_js_config() {
        ...
        ?>
        <script>
            var tbcFpConfig = <?php echo wp_json_encode([
                'fields'              => $frontendFields,
                'signupInstructions'  => $signupInstructions,
                'restUrl'             => rest_url('fluent-community/v2/'),
                'nonce'               => wp_create_nonce('wp_rest'),
                'ajaxUrl'             => admin_url('admin-ajax.php'),
                'ajaxNonce'           => wp_create_nonce('tbc_fp_nonce'),
                'version'             => TBC_FP_VERSION,
            ]); ?>;
        </script>
        <?php
    }
    ...
    public function inject_js() {
        ?>
        <script src="<?php echo esc_url(TBC_FP_URL . 'assets/js/profile-fields.js'); ?>?v=<?php echo esc_attr(TBC_FP_VERSION); ?>" defer="defer"></script>
        <?php
    }
