...
class FollowHandler
{
    ...
    public function getFollowersCount($profileData, $xprofile)
    {
        ...
        if ($canViewFollowers) {
            $profileData['followers_count'] = $xprofile->followers()->where('level', '>', 0)->whereHas('follower')->count();
        }

        $canViewFollowings = FollowerHelper::canViewFollowings($xprofile->user_id);
        if ($canViewFollowings) {
            $profileData['followings_count'] = $xprofile->follows()->where('level', '>', 0)->whereHas('followed')->count();
        }
