...
function readProjectState() {
  ...
  if (fileExists(PATHS.configTs)) {
    ...
    // Feature flags
    state.features = {};
    const boolFlags = ['DARK_MODE', 'PUSH_NOTIFICATIONS', 'MESSAGING', 'COURSES', 'CART', 'MULTI_REACTIONS'];
    ...
    for (const sub of ['POSTS', 'SPACES', 'COMMENTS']) {
...
function importModule(tarGzBuffer) {
  ...
  for (const file of moduleFiles) {
    ...
    const destPath = path.join(moduleDir, relPath);
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="field-group">
        ...
        <div class="field">
          <label>Version <span class="file-hint">app.json + package.json (4 places)</span></label>
          ...
          <span class="version-files">Updates: app.json (version + buildNumber + versionCode) + package.json</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="build-version-bar">
        <span class="build-version-label">Version</span>
        <input type="text" id="build-version" class="build-version-input" readonly>
        <button type="button" class="btn btn-sm" onclick="bumpVersion('major');syncBuildVersion()">Major</button>
        <button type="button" class="btn btn-sm" onclick="bumpVersion('minor');syncBuildVersion()">Minor</button>
        <button type="button" class="btn btn-sm" onclick="bumpVersion('patch');syncBuildVersion()">Patch</button>
...
function bumpVersion(type) {
  ...
  el.value = parts.join('.');
  // Mark dirty so Save Changes picks it up
  dirty = true;
  document.getElementById('save-config-btn').textContent = 'Save Changes *';
}
      ...
      if (isFinished) {
        ...
        if (isAndroid) {
          if (build.artifacts && build.artifacts.buildUrl) {
            actionsHtml += '<a class="btn btn-sm btn-primary" href="' + build.artifacts.buildUrl + '" target="_blank" rel="noopener">Download .aab</a>';
          }
