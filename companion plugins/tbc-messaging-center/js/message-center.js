...
jQuery(document).ready(function($) {
    ...
    function initSettingsTab() {
        $(document).off('click', '#tbc-mc-save-settings').on('click', '#tbc-mc-save-settings', function() {
            ...
            window.tbcMcAjax('tbc_mc_save_settings', {
                ...
                phone_field_slug: $('#tbc-mc-phone-field-slug').val(),
                sms_optin_field_slug: $('#tbc-mc-sms-optin-slug').val(),
                sms_optout_value: $('#tbc-mc-sms-optout-value').val()
            })
            .done(function(response) {
                if (response.success) {
    ...
    function openComposePanel() {
        ...
        if (contactSection.length && selectedRecipients.length > 0) {
            ...
            } else if (currentSendType === 'space') {
                summaryText += ' from: Space Members';
    ...
    $(document).on('click', '.tbc-mc-hierarchy-header', function() {
        ...
        if (children.is(':visible')) {
            ...
            if (!children.data('loaded')) {
                ...
                // Branch based on mode or data attribute
                const spaceId = item.data('space-id');
                if (spaceId) {
                    // Fluent Community Space - load members
                    const smsOptIn = $('#tbc-mc-space-sms-opt-in').is(':checked');
                    loadMembersForSpace(spaceId, children, smsOptIn);
    ...
    // ========================================================================
    // SPACES - Fluent Community Space Members
    // ========================================================================

    function loadSpaces() {
        const container = $('#tbc-mc-space-list');
        container.html('<div class="tbc-mc-loading">Loading spaces...</div>');

        tbcMcAjax('tbc_mc_fetch_spaces', {})
        .done(function(response) {
            if (response.success && response.data.data && response.data.data.spaces) {
                renderSpaceList(response.data.data.spaces);
            } else {
                container.html('<p class="tbc-mc-no-data">No spaces found</p>');
            }
        })
        .fail(function() {
            container.html('<p class="tbc-mc-error">Error loading spaces</p>');
        });
    }
    ...
    function renderSpaceList(spaces) {
        ...
        // Group spaces by their parent group
        let currentGroup = null;
        spaces.forEach(function(space) {
            const groupTitle = space.group_title || '';

            // Add group header if this is a new group
            if (groupTitle !== currentGroup) {
                currentGroup = groupTitle;
                if (groupTitle) {
                    $container.append(`<div class="tbc-mc-space-group-header">${escapeHtml(groupTitle)}</div>`);
                }
            }

            const memberLabel = space.member_count === 1 ? '1 member' : space.member_count + ' members';
            const searchTitle = ((groupTitle ? groupTitle + ' ' : '') + space.title).toLowerCase();
            const item = $(`
                <div class="tbc-mc-hierarchy-item tbc-mc-space-item" data-space-id="${space.id}" data-space-title="${escapeHtml(searchTitle)}">
                    <div class="tbc-mc-hierarchy-header">
                        <span class="tbc-mc-hierarchy-expand">▶</span>
                        <span class="tbc-mc-hierarchy-title">${escapeHtml(space.title)}</span>
                        <span class="tbc-mc-space-member-count">(${memberLabel})</span>
                    </div>
                    <div class="tbc-mc-hierarchy-children" style="display: none;"></div>
                </div>
            `);
    ...
    // Space search filter (client-side)
    $(document).on('input', '#tbc-mc-space-search', function() {
        const query = $(this).val().toLowerCase().trim();

        // Filter individual space items
        $('.tbc-mc-space-item').each(function() {
            const title = $(this).data('space-title') || '';
            $(this).toggle(title.indexOf(query) !== -1);
        });

        // Show/hide group headers based on whether any child spaces are visible
        $('.tbc-mc-space-group-header').each(function() {
            const $header = $(this);
            // Check if any following space items (before the next group header) are visible
            let hasVisible = false;
            $header.nextUntil('.tbc-mc-space-group-header').filter('.tbc-mc-space-item').each(function() {
                if ($(this).is(':visible')) {
                    hasVisible = true;
                    return false; // break
                }
            });
            $header.toggle(hasVisible || !query);
        });
    });

    // SMS Opt-In checkbox - reload expanded spaces when toggled
    $(document).on('change', '#tbc-mc-space-sms-opt-in', function() {
        // Reset all loaded space member lists so they reload with the new filter
        $('.tbc-mc-space-item .tbc-mc-hierarchy-children').each(function() {
            if ($(this).data('loaded')) {
                $(this).removeData('loaded').empty().hide();
                $(this).closest('.tbc-mc-hierarchy-item').find('.tbc-mc-hierarchy-expand').text('▶');
            }
        });
        // Reset selections
    ...
    function loadMembersForSpace(spaceId, container, smsOptIn) {
        const data = { space_id: spaceId };
        if (smsOptIn) {
            data.sms_opt_in = 'true';
        }

        tbcMcAjax('tbc_mc_fetch_space_members', data)
        .done(function(response) {
            container.data('loaded', true);
            if (response.success && response.data.data && response.data.data.memberList && response.data.data.memberList.length > 0) {
                const members = response.data.data.memberList;
                container.empty();
                ...
                const selectAll = $(`
                    <div class="tbc-mc-customer-select-all">
                        <label>
                            <input type="checkbox" class="tbc-mc-select-all-customers" data-space-id="${spaceId}">
                ...
                members.forEach(function(member) {
                    const isOptedOut = member.is_sms_out;
                    const optOut = isOptedOut ? ' <span class="tbc-mc-opted-out">(Opted Out)</span>' : '';
                    const disabled = isOptedOut ? ' disabled' : '';
                    const memberItem = $(`
                        <div class="tbc-mc-customer-item${isOptedOut ? ' opted-out' : ''}">
                            <label>
                                <input type="checkbox" class="tbc-mc-customer-check"
                                    ...
                                    ${disabled}>
                                <span class="tbc-mc-customer-name">${escapeHtml(member.name || 'Unknown')}</span>
                                <span class="tbc-mc-customer-phone">${escapeHtml(member.phone)}</span>
                                ${optOut}
                            </label>
                        </div>
                    `);
                ...
                container.html('<p class="tbc-mc-no-data">No members with phone numbers found</p>');
            }
        })
        .fail(function() {
            container.html('<p class="tbc-mc-error">Error loading members</p>');
        });
    }

    // Select all customers for a date, product, or space
    $(document).on('change', '.tbc-mc-select-all-customers', function() {
        const isChecked = $(this).prop('checked');
        let container = $(this).closest('.tbc-mc-date-customers');
        if (!container.length) {
            container = $(this).closest('.tbc-mc-hierarchy-children');
        }
