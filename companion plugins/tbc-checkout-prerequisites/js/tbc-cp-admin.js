(function($) {
    ...
    const TBC_CP_Admin = {
        ...
        populateSelects() {
            ...
            if (tbc_cp_admin.forms) {
                tbc_cp_admin.forms.forEach(form => {
                    $formSelect.append(`<option value="${form.id}">${form.title}</option>`);
                });
            }
        },
        ...
        validateAndSave() {
            ...
            const gvShortcode = $('#tbc-cp-gv-shortcode').val().trim();
            if (gvShortcode && !gvShortcode.includes('[gravityview')) {
                this.showNotice('Please enter a valid GravityView shortcode', 'error');
                return;
            }
        ...
        saveStep() {
            ...
            if (formData.type === 'course') {
                ...
                formData.gravityview_shortcode = $('#tbc-cp-gv-shortcode').val().trim();
        ...
        openModal(stepData = null) {
            ...
            if (stepData) {
                ...
                if (stepData.type === 'course') {
                    ...
                    $('#tbc-cp-form-id').val(stepData.form_id);
                    ...
                    if (stepData.gravityview_shortcode) {
                        $('#tbc-cp-gv-shortcode').val(stepData.gravityview_shortcode);
                    }
        ...
        generateStepHtml(step) {
            ...
            if (step.type === 'form') {
                ...
                if (step.gravityview_shortcode) {
                    metaHtml += '<span class="tbc-cp-meta-gv">Has GravityView</span>';
                }
