/**
 * TBC Fluent Profiles - Portal Profile Fields JS
 *
 ...
 *          custom_fields via X-TBC-FP-Fields header + body injection.
 *
...
(function () {
    ...
    var config = window.tbcFpConfig || {};
    var fields = config.fields || {};
    var xhrPatched = false;

    // Cached profile data (populated by XHR response interception)
    window._tbcFpProfileData = null;
    window._tbcFpProfileConfigs = null;
    ...
    function checkSignupForm() {
        ...
        if (signupForm.getAttribute('data-tbc-fp-signup')) return;
        signupForm.setAttribute('data-tbc-fp-signup', '1');
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            // Don't inject twice
            if (wrapper.querySelector('.tbc-fp-signup-instructions')) continue;

            var hint = document.createElement('div');
            hint.className = 'tbc-fp-signup-instructions';
    ...
    function checkProfileAbout() {
        ...
        if (!aboutWrap) return;
        if (aboutWrap.querySelector('.tbc-fp-fields')) return;

        var match = window.location.pathname.match(/\/u\/([^\/]+)/);
        if (!match) return;

        // Data already cached from XHR interception? Render immediately.
        if (window._tbcFpProfileData) {
            renderProfileFields(aboutWrap, window._tbcFpProfileData);
            return;
        }
    ...
    function fetchProfileAndRender(username, aboutWrap) {
        if (aboutWrap.getAttribute('data-tbc-fp-loading')) return;
        aboutWrap.setAttribute('data-tbc-fp-loading', '1');
        ...
        fetch(restUrl + 'profile/' + encodeURIComponent(username), {
            ...
            .catch(function (err) {
                console.warn('TBC FP: Could not fetch profile data', err);
            });
    }
    ...
    function renderProfileFields(aboutWrap, customFields) {
        ...
        wrapper.className = 'tbc-fp-fields';
    ...
    function checkEditForm() {
        ...
        if (!editForm) return;
        if (editForm.querySelector('.tbc-fp-edit-fields')) return;
    ...
    function injectEditSection(editForm) {
        ...
        var customFields = window._tbcFpProfileData || {};
        var fieldConfigs = window._tbcFpProfileConfigs || fields;

        // Build section matching native .fcom_sub_group structure
        var section = document.createElement('div');
        section.className = 'fcom_sub_group tbc-fp-edit-fields';
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (fieldDef.required) {
                labelHtml += ' <span class="tbc-fp-required">*</span>';
            }
            labelHtml += '</label>';
            ...
            if (fieldDef.instructions) {
                instructionsHtml = '<div class="tbc-fp-instructions">' + escapeHtml(fieldDef.instructions) + '</div>';
            }
    ...
    function buildInputHtml(key, fieldDef, value) {
        ...
        var dataAttr = 'data-tbc-fp-key="' + escapeAttr(key) + '"';
        ...
        switch (type) {
            ...
            case 'gender':
                ...
                return '<div class="el-select tbc-fp-select-wrap">' +
                    '<select class="el-input__inner" ' + dataAttr + '>' + optionsHtml + '</select>' +
                    '</div>';

            case 'radio':
                var radioOptions = fieldDef.options || [];
                var radioHtml = '<div class="tbc-fp-radio-group" ' + dataAttr + '>';
                for (var r = 0; r < radioOptions.length; r++) {
                    var checked = (radioOptions[r] === value) ? ' checked' : '';
                    var radioId = 'tbc-fp-' + key + '-' + r;
                    radioHtml += '<label class="tbc-fp-radio" for="' + radioId + '">' +
                        '<input type="radio" id="' + radioId + '" name="tbc_fp_' + escapeAttr(key) + '" ' +
            ...
            case 'multiselect':
                ...
                if (type === 'multiselect') {
                    var msHtml = '<div class="tbc-fp-multiselect-wrap">' +
                ...
                // Checkbox group
                var cbHtml = '<div class="tbc-fp-checkbox-group" ' + dataAttr + '>';
                for (var c = 0; c < cbOptions.length; c++) {
                    var cbChecked = (selectedValues.indexOf(cbOptions[c]) !== -1) ? ' checked' : '';
                    var cbId = 'tbc-fp-' + key + '-' + c;
                    cbHtml += '<label class="tbc-fp-checkbox" for="' + cbId + '">' +
    ...
    function buildVisibilityDropdown(key, currentVis) {
        var attr = 'data-tbc-fp-visibility="' + escapeAttr(key) + '"';
        var opts = [
        ...
        var html = '<div class="tbc-fp-visibility-wrap">' +
            '<label class="tbc-fp-visibility-label">Who can see this:</label>' +
            '<select class="tbc-fp-visibility-select" ' + attr + '>';
            ...
            if (info && info.url.indexOf('/profile') !== -1) {
                ...
                if (info.method === 'POST' || info.method === 'PUT' || info.method === 'PATCH') {
                    ...
                    if (Object.keys(cfValues).length) {
                        ...
                        try {
                            this.setRequestHeader('X-TBC-FP-Fields', JSON.stringify(cfValues));
                        } catch (e) {}
    ...
    function processProfileResponse(data) {
        ...
        if (customFields) {
            window._tbcFpProfileData = customFields;
            window._tbcFpProfileConfigs = profile.custom_field_configs || null;
        }
    }
    ...
    function collectEditFieldValues() {
        ...
        // Standard inputs, selects, textareas with data-tbc-fp-key
        var inputs = document.querySelectorAll('[data-tbc-fp-key]');
        for (var i = 0; i < inputs.length; i++) {
            var el = inputs[i];
            var key = el.getAttribute('data-tbc-fp-key');
            ...
            if (el.tagName === 'SELECT' && el.multiple) {
                ...
                values[key] = el.value;
            } else if (el.classList.contains('tbc-fp-radio-group')) {
                // Radio group: find checked radio
                var checked = el.querySelector('input[type="radio"]:checked');
                values[key] = checked ? checked.value : '';
            } else if (el.classList.contains('tbc-fp-checkbox-group')) {
        ...
        var visSelects = document.querySelectorAll('[data-tbc-fp-visibility]');
        for (var v = 0; v < visSelects.length; v++) {
            var visEl = visSelects[v];
            var visKey = visEl.getAttribute('data-tbc-fp-visibility');
