...
class SmsRoles {
     ...
     * SMS opt-in profile field value.
     *
    ...
    public function sync_sms_role( $user_id, $context = '' ) {
        ...
        $field_key = Helpers::get_option( 'sms_optin_field', '' );
        if ( empty( $field_key ) ) {
        ...
        $meta_key    = TBC_FP_META_PREFIX . $field_key;
        ...
        Helpers::log( "[SMS Roles] [{$context}] User {$user_id}: meta_key={$meta_key}, value='{$current}', optin_value='{$optin_value}'" );
    ...
    /**
     * Hook: tbc_fp_after_ajax_save (fires inside ajax_save_fields before wp_send_json).
     *
     * @param int   $user_id The user whose fields were saved.
     * @param array $saved   The saved field values.
     */
    public function on_ajax_save( $user_id, $saved ) {
        $this->sync_sms_role( $user_id, 'ajax' );
    }

    /**
     * Hook: tbc_fp_registration_response (priority 5).
