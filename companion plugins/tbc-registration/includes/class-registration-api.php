 ...
 *
 * Endpoints:
 *   GET  /tbc-reg/v1/register/fields  - Get registration form field definitions
 *   POST /tbc-reg/v1/register         - Submit registration (handles OTP + email verify flow)
 *
...
class RegistrationApi {
    ...
    public function register_routes() {
        register_rest_route(TBC_REG_REST_NAMESPACE, '/register/fields', [
            'methods'             => 'GET',
            'callback'            => [$this, 'handle_get_fields'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route(TBC_REG_REST_NAMESPACE, '/register', [
            'methods'             => 'POST',
            'callback'            => [$this, 'handle_register'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route(TBC_REG_REST_NAMESPACE, '/register/complete', [
            'methods'             => 'POST',
        ...
        register_rest_route(TBC_REG_REST_NAMESPACE, '/register/status', [
            'methods'             => 'GET',
    ...
    public function handle_get_fields(\WP_REST_Request $request) {
        ...
        // FC native custom profile fields (all enabled fields appear on registration)
        $fc_field_defs = Helpers::get_fc_field_definitions();
    ...
    public function handle_register(\WP_REST_Request $request) {
        ...
        $session_key = sanitize_text_field($data['tbc_reg_session_key'] ?? '');
        ...
        if ($otp_enabled) {
            ...
            if (!empty($session_key) && Helpers::is_verified($session_key)) {
                ...
                if (!empty($phone_value)) {
                    ...
                    $new_session_key = Helpers::generate_session_key('tbc_reg_session_');
        ...
        if (!empty($values_to_save)) {
            Helpers::set_fc_custom_fields($user_id, $values_to_save);
        }
         ...
         */
        $response_data = apply_filters('tbc_reg_registration_response', $response_data, $user_id, $request);
