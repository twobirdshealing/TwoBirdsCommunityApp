 ...
 *
 * Steps:
 *   1. All fields (name, email, username, password, custom fields, terms)
 *   2. Email verification (if FC 2FA enabled)
 *   3. Phone OTP (if enabled)
 *
 * Steps 5-6 (bio + avatar) are handled by profile-completion.js
 * via the [tbc_profile_completion] shortcode on the same page.
 *
 * Calls the same REST API as the mobile app (tbc-reg/v1/register/*).
...
(function () {
    ...
    const API = config.restUrl;       // e.g. https://site.com/wp-json/tbc-reg/v1/
    ...
    async function submitRegistration(otpKey, emailCode, emailToken) {
        globalError = '';
        if (!otpKey && !emailCode && !validateStep(1)) { render(); return; }
        ...
        try {
            ...
            var eCode = emailCode || verifiedEmailCode;
            var eToken = emailToken || verifiedEmailToken;
            if (eCode && eToken) {
                payload.__two_fa_code = eCode;
                payload.__two_fa_signed_token = eToken;
            }

            if (otpKey) {
                payload.tbc_reg_session_key = otpKey;
            }
            ...
            if (result.otp_required && result.session_key) {
                // Store email credentials so OTP resubmit includes them
                if (eCode && eToken) {
                    verifiedEmailCode = eCode;
                    verifiedEmailToken = eToken;
                }
            ...
            // Errors — all fields are on step 1
            if (result.errors) {
    ...
    function render() {
        ...
        // Title
        var titles = { 1: 'Create Account', 3: 'Verify Email', 4: 'Verify Phone' };
    ...
    function getVisualStep(s) {
        // Step 2 is removed — map step 3→2, step 4→3 (adjusted for skipped verification steps)
        var v = s;
        if (s >= 2) v--; // no step 2 anymore
    ...
    function renderStep3() {
        ...
        html += '</button>';
        html += '</div>';
        html += '<button type="button" class="tbc-reg__link" data-action="back-to-1">Go Back</button>';
        return html;
    }
    ...
    function renderStep4() {
        ...
        html += '</div>';
        html += '<button type="button" class="tbc-reg__link" data-action="back-to-step3or1">Go Back</button>';
        return html;
    }
    ...
    function renderField(key, field) {
        ...
        switch (field.type) {
            ...
            default: {
                ...
                else if (key.indexOf('phone') !== -1) inputType = 'tel';
    ...
    function handleAction(action) {
        ...
        switch (action) {
            ...
            case 'back-to-1':
                step = 1; formData._emailCode = ''; render(); break;
            case 'verify-email':
                verifyEmail(); break;
            case 'resend-email':
            ...
            case 'back-to-step3or1':
                step = fieldsConfig?.email_verification_required ? 3 : 1;
