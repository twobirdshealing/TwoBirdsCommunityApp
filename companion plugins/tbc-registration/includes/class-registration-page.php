 ...
 *   4-5. Profile completion: social links + avatar (post-login, optional)
 *
...
class RegistrationPage {
    ...
    public function render_shortcode($atts = []) {
        ...
        if (is_user_logged_in()) {
            if ($this->is_registration_complete()) {
                ...
                return $inline_assets . '<div class="tbc-reg-already-logged-in">'
                    . '<p>' . esc_html__('You are already logged in.', 'tbc-registration') . '</p>'
                    . '<a href="' . esc_url($community_url) . '">' . esc_html__('Go to Community', 'tbc-registration') . '</a>'
                    . '</div>';
            }
        ...
        $config = [
            'restUrl'      => rest_url(TBC_REG_REST_NAMESPACE . '/'),
        ...
        <div id="tbc-registration-app" class="tbc-reg" data-config="<?php echo esc_attr(wp_json_encode($config)); ?>">
            <div class="tbc-reg__loading">
                ...
                <p><?php esc_html_e('Loading registration form...', 'tbc-registration'); ?></p>
            </div>
        </div>
        <?php
        return ob_get_clean();
    ...
    public function render_profile_completion_shortcode($atts = []) {
        ...
        $js_url = esc_url(TBC_REG_URL . 'assets/js/profile-completion.js?ver=' . TBC_REG_VERSION);
        ...
        $output .= '<div id="tbc-profile-completion-app" class="tbc-reg" data-config="' . esc_attr(wp_json_encode($config)) . '">'
            . '<div class="tbc-reg__loading">'
            . '<div class="tbc-reg__spinner"></div>'
            . '<p>' . esc_html__('Loading...', 'tbc-registration') . '</p>'
            . '</div>'
            . '</div>';
    ...
    private function get_inline_assets() {
        ...
        $css_file = TBC_REG_DIR . 'assets/css/registration.css';
        ...
        $js_url = esc_url(TBC_REG_URL . 'assets/js/registration.js?ver=' . TBC_REG_VERSION);
        $output .= '<script src="' . $js_url . '" defer="defer"></script>';
