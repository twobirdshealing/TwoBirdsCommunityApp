...
// Only load if WooCommerce is active and integration is enabled
...
/**
 * Add mini cart to Fluent Community header
 */
if (fluent_starter_show_mini_cart()) {
    add_action('fluent_community/before_header_right_menu_items', 'fluent_starter_render_mini_cart', 10, 1);
}

/**
 * ==========================================================================
 * Shared CSS/JS Helpers (single source of truth — no duplication)
 * ==========================================================================
 */
 ...
 *
 * Uses a static guard to prevent double output when both
 * portal_head and wp_head fire on the same page load.
 */
function fluent_starter_output_mini_cart_css() {
    static $output = false;
    if ($output) return;
    $output = true;
    ?>
 ...
 */
function fluent_starter_output_mini_cart_js() {
    static $output = false;
    if ($output) return;
    $output = true;
    ...
    (function() {
        ...
        document.addEventListener('click', function(e) {
            ...
            .then(function(data) {
                if (data.fragments) {
                    // Replace mini cart HTML with updated fragment
                    ...
                    // Re-initialize dropdown after fragment replacement
                    initCartDropdown();
                }
            })
            .catch(function() {
 ...
 *
 * Shared between portal_head and inline rendering contexts.
 * Uses !important to override WooCommerce defaults within the Fluent frame.
 */
function fluent_starter_output_wc_component_css() {
    static $output = false;
    if ($output) return;
    $output = true;
    ?>
 ...
 */
function fluent_starter_get_gamipress_credits_html() {
    if (!fluent_starter_show_gamipress()) return '';
    if (!function_exists('gamipress_get_user_points')) return '';
    if (!is_user_logged_in()) return '';

    $user_id = get_current_user_id();

    // Get first available point type
    ...
    // Get the point type post ID and thumbnail
     ...
     */
    $achievements_url = apply_filters('fluent_starter_gamipress_url', home_url('/'));
 ...
 */
function fluent_starter_render_mini_cart($auth) {
    if (!WC()->cart) {
        return;
    }
    ...
    <li class="top_menu_item fs_mini_cart_wrap fcom_countable_notification_holder">
        <div class="fs_cart_extend">
            ...
            <div class="fs_cart_dropdown">
                <?php echo fluent_starter_get_gamipress_credits_html(); ?>
                <?php if ($cart_count > 0): ?>
                    <div class="fs_cart_items">
                        ...
                        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                            ...
                            <div class="fs_cart_item">
                                ...
                                <a href="<?php echo esc_url(wc_get_cart_remove_url($cart_item_key)); ?>" class="fs_cart_item_remove" data-cart-item-key="<?php echo esc_attr($cart_item_key); ?>" aria-label="<?php esc_attr_e('Remove item', 'fluent-starter'); ?>"><svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><line x1="2" y1="2" x2="10" y2="10"/><line x1="10" y1="2" x2="2" y2="10"/></svg></a>
                            </div>
                            <?php
                        }
                        ?>
                    ...
                    <div class="fs_cart_footer">
                        <div class="fs_cart_subtotal">
                            <span><?php esc_html_e('Subtotal:', 'fluent-starter'); ?></span>
                            <strong><?php echo wp_kses_post($cart_total); ?></strong>
                        </div>
                        <div class="fs_cart_buttons">
                            <a href="<?php echo esc_url(wc_get_cart_url()); ?>" class="fs_btn fs_btn_secondary"><?php esc_html_e('View Cart', 'fluent-starter'); ?></a>
                            <a href="<?php echo esc_url(wc_get_checkout_url()); ?>" class="fs_btn fs_btn_primary"><?php esc_html_e('Checkout', 'fluent-starter'); ?></a>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="fs_cart_empty">
                        <p><?php esc_html_e('Your cart is empty.', 'fluent-starter'); ?></p>
                        <a href="<?php echo esc_url(wc_get_page_permalink('shop')); ?>" class="fs_btn fs_btn_primary"><?php esc_html_e('Browse Events', 'fluent-starter'); ?></a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
/**
 * Output mini cart CSS + JS and WooCommerce component styles on portal pages
 * (Fluent Community frame — portal_head fires for both portal and frame pages)
 */
add_action('fluent_community/portal_head', function() {
    if (fluent_starter_show_mini_cart()) {
        fluent_starter_output_mini_cart_css();
        fluent_starter_output_mini_cart_js();
    }
    fluent_starter_output_wc_component_css();
});

/**
 * Enqueue mini cart assets on regular WP pages (not just portal)
 * This ensures the CSS/JS loads on non-portal pages where wp_head/wp_footer run
 */
add_action('wp_enqueue_scripts', function() {
    ...
    add_action('wp_head', 'fluent_starter_output_mini_cart_css', 99);
    add_action('wp_footer', 'fluent_starter_output_mini_cart_js', 99);
});

/**
 * ==========================================================================
 * WooCommerce Content Rendering in Fluent Frame
 * ==========================================================================
 */

/**
 * Register WooCommerce content handler at wp_loaded
 *
 * This approach is more reliable than removing handlers during rendering because:
 * 1. Handler removal happens BEFORE any rendering begins
 * 2. All plugins have finished loading at wp_loaded
 * 3. Prevents race conditions with other plugins
 *
 * Priority 25 runs after fluent-integration.php's wp_loaded (priority 20)
 */
add_action('wp_loaded', function() {
    ...
    // Remove the default theme content handler at priority 10
    // This prevents double output from fluent_starter_render_theme_content
    remove_action('fluent_community/theme_content', 'fluent_starter_render_theme_content', 10);

    // Add our unified handler that routes to appropriate content renderer
    add_action('fluent_community/theme_content', 'fluent_starter_unified_render_content', 10, 2);
}, 25);

/**
 * Unified content renderer for Fluent Community frame
 *
 * Routes to appropriate handler based on page type:
 * - WooCommerce pages -> WooCommerce content
 * - Blog pages -> Blog content
 * - Other pages -> Default page content
 *
 * @param string $themeName The active theme name
 * @param string $wrapperType The wrapper type ('default' or 'full')
 */
function fluent_starter_unified_render_content($themeName, $wrapperType = 'default') {
    // Route all WooCommerce pages (shop, product, cart, checkout, account) to WC renderer
    if (function_exists('is_woocommerce') && (is_woocommerce() || is_cart() || is_checkout() || is_account_page())) {
        fluent_starter_render_wc_page_content($wrapperType);
        return;
    }

    // All other pages use default theme rendering
    fluent_starter_render_theme_content($themeName, $wrapperType);
}
 ...
 * @param string $wrapperType The wrapper type ('default' or 'full')
 */
function fluent_starter_render_wc_page_content($wrapperType = 'default') {
    // Output WC component styles (shared helper handles deduplication)
    fluent_starter_output_wc_component_css();
    ...
    // Shop/product pages use woocommerce_content() for proper template loading
    // Cart, checkout, and account are WP pages with WC shortcodes — use the_content()
 ...
 */
add_filter('woocommerce_add_to_cart_fragments', 'fluent_starter_cart_fragments');

function fluent_starter_cart_fragments($fragments) {
    ob_start();
    fluent_starter_render_mini_cart([]);
