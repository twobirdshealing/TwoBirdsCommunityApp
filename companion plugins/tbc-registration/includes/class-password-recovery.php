...
class PasswordRecovery {
    ...
    public function intercept_lost_password(\WP_Error $errors): void {
        ...
        // Look up phone from FC native custom fields
        $raw_phone = Helpers::get_phone_value($user->ID);
        ...
        $session_key = Helpers::generate_session_key('tbc_reg_recovery_');
    ...
    public function show_error_message(string $message): string {
        ...
        if (isset($_GET['otp_error']) && $_GET['otp_error'] === '1') {
            $error = '<div id="login_error"><strong>' .
                esc_html__('Unable to send verification code. Please try again or contact support.', 'tbc-registration') .
                '</strong></div>';
    ...
    private function render_recovery_page(string $session_key, string $phone): void {
        ...
        include TBC_REG_DIR . 'templates/otp-modal.php';
    }
