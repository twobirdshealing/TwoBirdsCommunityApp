<?php
/**
 * Book Club API - REST endpoints for the Book Club audiobook player
 *
 * Reads from the TBC Book Club plugin's database tables (wp_tbc_bc_books,
 * wp_tbc_bc_bookmarks) to provide the mobile app with book data, chapters,
 * and bookmark management.
 *
 * The web player uses WordPress AJAX; these REST endpoints serve the same
 * data to the app via JWT-authenticated requests.
 */
...
class TBC_CA_Book_Club_API {
    ...
    public function register_routes() {
        // GET /tbc-ca/v1/books  (?current=1 returns only the current book)
        register_rest_route(TBC_CA_REST_NAMESPACE, '/books', [
            'methods'             => 'GET',
        ...
        // GET /tbc-ca/v1/books/{id}
        register_rest_route(TBC_CA_REST_NAMESPACE, '/books/(?P<id>\d+)', [
            'methods'             => 'GET',
        ...
        // POST /tbc-ca/v1/books/{id}/bookmarks
        register_rest_route(TBC_CA_REST_NAMESPACE, '/books/(?P<id>\d+)/bookmarks', [
            'methods'             => 'POST',
        ...
        // DELETE /tbc-ca/v1/books/{id}/bookmarks/{bookmark_id}
        register_rest_route(TBC_CA_REST_NAMESPACE, '/books/(?P<id>\d+)/bookmarks/(?P<bookmark_id>\d+)', [
            'methods'             => 'DELETE',
