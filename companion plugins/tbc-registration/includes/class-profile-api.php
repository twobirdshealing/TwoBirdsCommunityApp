...
class ProfileApi {
     ...
     * Reads custom_fields from the request body or X-TBC-REG-Fields header.
     *
    ...
    public function handle_profile_update($updateData, $data, $xProfile) {
        ...
        if (empty($customFieldValues)) {
            $headerValue = isset($_SERVER['HTTP_X_TBC_REG_FIELDS']) ? $_SERVER['HTTP_X_TBC_REG_FIELDS'] : '';
            if ($headerValue) {
     ...
     * Called via admin-ajax.php?action=tbc_reg_save_fields
     */
    public function ajax_save_fields() {
        if (!check_ajax_referer('tbc_reg_nonce', '_nonce', false)) {
         ...
         */
        do_action('tbc_reg_after_ajax_save', $targetUserId, $saved);
