...
All notable changes to the TBC Registration plugin (formerly TBC Fluent Profiles).

## v4.0.0
- **BREAKING: Renamed plugin from `tbc-fluent-profiles` to `tbc-registration`**
- All prefixes renamed: `tbc_fp_` → `tbc_reg_`, `TBC_FP_` → `TBC_REG_`, namespace `TBCFluentProfiles` → `TBCRegistration`
- REST namespace changed from `tbc-fp/v1` to `tbc-reg/v1`
- Plugin folder/file: `tbc-fluent-profiles/tbc-fluent-profiles.php` → `tbc-registration/tbc-registration.php`
- Automatic database migration on activation: renames all `tbc_fp_*` options and `_tbc_fp_*` user meta to new prefixes
- Removed entire Bot Protection system (Turnstile, App Token, disposable email blocking)
- Phone field now uses `type="tel"` keyboard on web registration
- Simplified registration to single step (from v3.0.1)
- App config: `TBC_FP_URL` → `TBC_REG_URL`, `tbc_fp_session_key` → `tbc_reg_session_key`
- Updated setup dashboard, setup guide, snapshot script, and all docs

## v3.0.3
- **Removed entire Bot Protection system** — Turnstile, App Token, disposable email blocking, and blocked emails list all removed. Email verification + phone OTP provide sufficient protection.
- Removed Bot Protection admin tab and all related settings
- Removed `APP_TOKEN` from app config, `X-App-Token` header from registration API client
- Removed Bot Protection card from setup dashboard, step from setup guide
- Cleaned up activation hook, uninstall handler, and helpers
- Phone field now uses `type="tel"` keyboard on web registration

## v3.0.1
- **Simplified registration to single step** — all fields (name, email, username, password, custom fields, terms) now on one screen. Removed step 2 from both web and app registration flows.
- Fixed `is_required` property name in registration validation (was checking `required` which doesn't exist on FC native fields)
- Fixed `show_on_signup` flag added to migration tool field definitions so custom fields appear during registration
- Registration JS: removed step 2 rendering, step indicator starts at 1 step, back navigation updated

## v3.0.0
- **MAJOR: Migrated to FC native custom profile fields** — profile fields are now managed in Fluent Community Pro → Settings → Custom Profile Fields. This plugin no longer manages field definitions, storage, or display.
- **Removed**: Fields class, Visibility class, ProfileApi class, Frontend class, admin Profile Fields tab, field editor modal, portal JS/CSS injection, wp_usermeta field storage, AJAX save path (`wp_ajax_tbc_fp_save_fields`)
- **Kept**: OTP verification (Twilio), multi-step registration REST API, bot protection (Turnstile, disposable email blocking, app token), profile completion gate, SMS role management, password recovery OTP
- Registration now reads FC native field definitions and saves to `fcom_xprofile.custom_fields` JSON (instead of `wp_usermeta`)
- Phone lookup, duplicate check, and OTP phone detection now read from FC native JSON column (`JSON_EXTRACT`)
- SMS role management reads opt-in value from FC native custom fields
- Password recovery reads phone from FC native custom fields
- Admin phone selector and SMS opt-in selector now list FC native fields (no more auto-detect from old Fields class)
- New setting `phone_field_slug` replaces old `phone_meta_key` / `phone_meta_key_custom`
- Plugin description updated to reflect new scope
- Requires FluentCommunity Pro 2.3.0+ with `custom_profile_fields` feature enabled
