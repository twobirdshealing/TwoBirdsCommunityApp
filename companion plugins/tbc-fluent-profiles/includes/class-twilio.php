...
class Twilio {
    ...
    public function check_verification(string $phone, string $code): array {
        ...
        if (!$creds || empty($service_sid)) {
            return $this->error_response(__('Twilio Verify credentials not configured.', 'tbc-fluent-profiles'), false);
        }
        ...
        if (is_wp_error($response)) {
            ...
            return $this->error_response(
                __('Unable to verify code. Please try again.', 'tbc-fluent-profiles'),
                false
            );
        }
        ...
        if ($status_code >= 400 || !is_array($body)) {
            ...
            $msg = $body['message'] ?? __('Verification check failed.', 'tbc-fluent-profiles');
        ...
        return [
            'success' => true,
            'message' => $approved
                ? __('Verification successful.', 'tbc-fluent-profiles')
                : __('Invalid verification code.', 'tbc-fluent-profiles'),
            'data'    => [
                'valid'  => $approved,
                'status' => $body['status'] ?? '',
            ],
    ...
    private function start(string $phone, string $channel): array {
        ...
        if (!$creds || empty($service_sid)) {
            return $this->error_response(__('Twilio Verify credentials not configured.', 'tbc-fluent-profiles'));
        }

        $formatted = Helpers::format_phone($phone);
        if (empty($formatted)) {
            return $this->error_response(__('Please enter a valid phone number.', 'tbc-fluent-profiles'));
        }
        ...
        if (is_wp_error($response)) {
            ...
            return $this->error_response(
                $channel === 'call'
                    ? __('Unable to initiate call. Please try again.', 'tbc-fluent-profiles')
                    : __('Unable to send verification. Please try again.', 'tbc-fluent-profiles')
            );
        }
        ...
        if ($status_code >= 400 || !is_array($body)) {
            $msg = $body['message'] ?? __('Failed to start verification.', 'tbc-fluent-profiles');
        ...
        return [
            ...
            'message' => $channel === 'call'
                ? __('Voice call initiated successfully.', 'tbc-fluent-profiles')
                : __('Verification code sent successfully.', 'tbc-fluent-profiles'),
            'data'    => [
